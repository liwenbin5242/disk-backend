 ...
 * @returns
 */
async function bindDisk(username, code, redirect_url) {
  const returnData = {};
  redirect_url = urldecode(redirect_url);
  const app = await diskDB.collection('baidu_apps').findOne({ redirect_url });
  const { data } = await utils.bdapis.code2token(
    code,
    app.app_key,
    app.secret_key,
    redirect_url
  ...
  const _id = ObjectID(
    await utils.md5ID(`${app.app_id}${username}|${bduserinfo.data.uk}`)
  );
...
async function updateUserInfo(
  ...
  await diskDB.collection('users').updateOne(
    { username },
    {
      $set: {
        avatar,
...
async function postShare(
  ...
  const user = await diskDB
    .collection('users')
    .findOne({ username, expires: { $gte: new Date() } });
  if (!user) {
  ...
  if (disk_id) {
    const disk = await diskDB
      .collection('disks')
      .findOne({ _id: ObjectID(disk_id), username });
    if (!disk) {
  ...
  await diskDB.collection('share_files').insertOne({
    username,
    disk_id,
    status,
    title,
...
async function putShare(
  ...
  await diskDB.collection('share_files').updateOne(
    { _id: ObjectID(_id) },
    {
      $set: {
        disk_id,
...
async function deleteRecursive(parent_id) {
  ...
  await diskDB
    .collection('share_files')
    .deleteOne({ _id: ObjectID(parent_id) });
}
...
async function getShare(username) {
  ...
  returnData.list = utils.array2Tree(
    share_disks,
    '_id',
    'parent_id',
    'children'
...
async function putBanners(
  ...
  const returnData = {};
  await diskDB.collection('banners').update(
    { _id: ObjectID(id) },
    {
      $set: {
        img_url,
...
async function postBanners(
  ...
  const returnData = {};
  await diskDB.collection('banners').insertOne({
    username,
    code,
    ctm: new Date(),
    status,
