 ...
 * @param {*} username
 */
async function putBanners(id ,status, img_url, redirect_url, sort , type, position, share_file_id, ) {
    const returnData = {};
    await diskDB.collection('banners').update({_id: ObjectID(id)}, {$set:{img_url, status, redirect_url, sort, type, position, share_file_id,  utm: new Date}});
    return returnData;
}
 ...
 * @param {*} username
 */
async function postBanners(username, code, status, img_url, redirect_url, sort, type, position, share_file_id) {
    const returnData = {};
    await diskDB.collection('banners').insertOne( {username, code, ctm: new Date, status, img_url, redirect_url, sort, type, position, share_file_id, utm: new Date});
    return returnData;
}
