...
router.get('/cdkey/list',  reqHandler(async function(req, res) {
    const {limit, offset, classify_id, key, used, sessionname, config_id, actived} = req.query
    const {username} = req.user
    const result = await robotServ.getCDkeyList( limit, offset,classify_id, key, used, (sessionname), config_id, username, actived)
    return res.json({code: returnCode.SUCCESS, data: result, message: 'ok'});
}));
